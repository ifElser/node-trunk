...
function Trunk(){
	...
	Array.prototype.forEach.call(arguments, arg => {
		...
		else if(arg instanceof stream.Duplex) this.setTrunk(arg);
	});
};
...
Trunk.prototype.setTrunk = function(trunk){

	this.trunk = trunk;
	let isChunk = false, chunkSize = 0, chunkId = 0, buffer = new Buffer(0);

	trunk.on('data', data => {
		let chunk;
		if(data.length) while(true) {
			if(isChunk){
